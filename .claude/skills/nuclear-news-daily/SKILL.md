...
## 3. 수집 — 우리 사업 관련 (비공개 워치리스트)

작업 공간에 `pipeline-watch` 저장소가 함께 클론돼 있으면 이 단계를 수행한다. 없으면 건너뛴다.

- `pipeline-watch/config/watchlist.yaml`을 읽는다. 권역(유럽·중동·북미·아시아)마다 추적 대상과 `watch` 키워드, 공고 사이트(`tender_sources`)가 있다.
- 키워드를 검색하고 `tender_sources`의 공고 페이지를 열어 **기간 내 신규 공고·입찰·파트너 동향**을 찾는다.
- **이 저장소의 내용은 사내 정보다.**
  - 워치리스트의 사업명·파트너·단계를 `nuclear-news-daily`의 공개 파일(`data/`, `reports/`, 웹페이지)에 **절대 쓰지 않는다.**
  - 공개 브리핑에는 기사 자체만 싣는다. "우리가 추진 중"이라는 맥락은 비공개 쪽에만 적는다.
  - 검색어로 쓰는 것은 괜찮다(검색 기록은 남지 않는다). 다만 검색 결과를 공개 파일에 옮길 때 워치리스트에서 온 맥락은 지운다.
- 찾은 건은 `pipeline-watch/alerts/DATE.md`에 기록한다:

```markdown
# 파이프라인 관련 동향 — YYYY-MM-DD

## 유럽
### [긴급/주의/참고] <대상 사업> — <한 줄 요약>
- 출처: <매체·기관> (YYYY-MM-DD) <URL>
- 내용: 2~3문장
- 우리 사업과의 관계: <워치리스트의 어느 대상에 어떻게 걸리는지>
- 제안 대응: <담당자가 확인할 것 1~2가지>

## 중동 / 북미 / 아시아
(같은 형식, 해당 없으면 "해당 없음")
```

- 등급은 **긴급**(입찰 공고 게시·마감 임박·파트너사 중대 변경), **주의**(발주처 일정·정책 변화), **참고**(배경 동향).
- 해당 건이 하나도 없으면 파일에 날짜와 "해당 없음"만 적는다. 빈 날도 기록해야 탐지율을 잴 수 있다.
- 커밋: `cd pipeline-watch && git add alerts && git commit -m "watch: DATE (N건)" && git push`
  - **두 저장소를 헷갈리지 않는다.** 공개 저장소에는 브리핑만, 비공개 저장소에는 알림만 커밋한다.
